...
// Parse request metadata from the same accepted JSON body size we already allow for request
// transforms so large conversations keep their stream/model flags.
