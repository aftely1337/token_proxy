...
#[test]
fn responses_request_buffers_codex_sse_into_json_response() {
    run_async(async {
        let codex = spawn_mock_raw_upstream(
            StatusCode::OK,
            Bytes::from(
                "event: response.created\n\
data: {\"type\":\"response.created\",\"response\":{\"id\":\"resp_buffered\",\"created_at\":123,\"model\":\"gpt-5-codex\"}}\n\n\
event: response.completed\n\
data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_buffered\",\"object\":\"response\",\"created_at\":123,\"model\":\"gpt-5-codex\",\"status\":\"completed\",\"output\":[{\"type\":\"message\",\"id\":\"msg_1\",\"status\":\"completed\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"buffered codex response\"}]}],\"usage\":{\"input_tokens\":1,\"output_tokens\":2,\"total_tokens\":3}}}\n\n\
data: [DONE]\n\n",
            ),
            "text/event-stream",
        )
        ...
        let data_dir = next_test_data_dir("responses_codex_buffered_sse");
        let state = build_test_state_handle(config, data_dir.clone()).await;
        ...
        let response_bytes = to_bytes(response.into_body(), usize::MAX)
            .await
            .expect("proxy buffered response bytes");
        let response_json: Value =
            serde_json::from_slice(&response_bytes).expect("proxy buffered response json");
        let requests = codex.requests();
        ...
        assert_eq!(response_json["id"], json!("resp_buffered"));
        assert_eq!(
            response_json["output"][0]["content"][0]["text"],
            json!("buffered codex response")
        );
        assert_eq!(requests.len(), 1);
